...
| 字段                         | 类型   | 默认值     | 说明                         |
| ---------------------------- | ------ | ---------- | ---------------------------- |
| Driver                       | Driver | `local`    | 存储驱动                     |
| Local.FSType                 | FSType | `FSTypeOS` | 本地文件系统类型             |
| Local.BasePath               | string | `.`        | 基础路径 (basepath 类型使用) |
| Local.EnableWatch            | bool   | `true`     | 是否启用文件监听             |
| Local.WatchBufferSize        | int    | `100`      | 监听事件缓冲区大小           |
...
export STORAGE_LOCAL_FS_TYPE=os
export STORAGE_LOCAL_BASE_PATH=/var/data
export STORAGE_LOCAL_ENABLE_WATCH=true
export STORAGE_LOCAL_WATCH_BUFFER_SIZE=200
```
